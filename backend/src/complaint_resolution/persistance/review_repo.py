...
def save_pending_review(state_dict: dict) -> str:
    ...
    conn = get_connection()
    conn.execute(
        "INSERT INTO pending_reviews (review_id, state_json, status, created_at) VALUES (%s, %s, %s, %s)",
...
def load_pending_review(review_id: str) -> dict | None:
    conn = get_connection()
    row = conn.execute(
        "SELECT state_json, status FROM pending_reviews WHERE review_id = %s", (review_id,)
    ).fetchone()
    conn.close()
    if row is None:
        return None
...
def mark_review_resolved(review_id: str):
    conn = get_connection()
    conn.execute("UPDATE pending_reviews SET status = 'resolved' WHERE review_id = %s", (review_id,))
    conn.commit()
    conn.close()
...
def update_pending_review(review_id: str, state_dict: dict):
    conn = get_connection()
    conn.execute(
        "UPDATE pending_reviews SET state_json = %s WHERE review_id = %s",
